...
class LinearDecoder(BaseEstimator):
	"""
	LinearDecoder is a special sparse autoencoder where activation function 
	in the output layer is an identitfy function, so the name.
	It has certain advantage over a Sparse Autoencoder because now the Z output 
	is not limited to [0, 1], so the inputs are not requried to be scaled to 
	[0, 1] before fed to a sparse autoencoder. This is specially useful when 
	an appropriate scaling of input is not in place, e.g. for PCA whitened data, 
	or for color images. 
	"""
	def __init__(self, n_vis, n_hid, sparsity, l2_coeff, sparsity_coeff):
		"""
